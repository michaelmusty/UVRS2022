...
path_to_5k_raw = "input_data/race_data_raw/03_RedWhiteBlue_10k_20220704/5k_raw.csv"
path_to_10k_raw = "input_data/race_data_raw/03_RedWhiteBlue_10k_20220704/10k_raw.csv"
...
def _process_path(path: str, which_race: str):
    ...
    with open(path, newline="") as csvfile:
        ...
        for row in reader:
            if row[0] == "Place" or row[9] == "DNF" or row[9] == "DNS":
            ...
            net = _parse_net_time(row[9])
            d["First Name"].append(first)
            d["Last Name"].append(last)
            d["Net Time"].append(net)
    df = pd.DataFrame(d)
    logger.info(df)
    df.to_csv(f"input_data/race_data/03_RedWhiteBlue_10k_20220704/{which_race}.csv")
